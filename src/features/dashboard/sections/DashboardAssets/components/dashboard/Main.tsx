...
function MainTab({ viewMode, currentTab }: Props) {
   ...
   const handleDeleteAsset = (id: string) => {
      setAssets((prev) =>
         prev.map((asset) => (asset.id === id ? { ...asset, status: 'deleted' } : asset)),
      )
   }

   const handleRestoreAsset = (id: string) => {
      setAssets((prev) =>
         prev.map((asset) => (asset.id === id ? { ...asset, status: 'active' } : asset)),
      )
   }
   ...
   return (
      <main className="flex-1 flex flex-col p-2 sm:p-4 overflow-auto container mx-auto max-w-full">
         ...
         <section className="mb-6">
            <h2 className="text-lg sm:text-xl font-semibold mb-3">Browse Categories</h2>
